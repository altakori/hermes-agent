...
def test_disable_credential_tombstones_and_removes_profile_row(profile_env):
    from hermes_cli.auth import disable_credential, read_credential_pool

    _write(profile_env["global"] / "auth.json", _make_auth_store(pool={}))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(pool={
        "openai-codex": [
            {"id": "remove-me", "label": "removed", "access_token": "old"},
            {"id": "keep-me", "label": "kept", "access_token": "good"},
        ],
    }))

    assert disable_credential("openai-codex", "remove-me") is True
...
def test_global_tombstone_filters_profile_and_stale_writer(profile_env):
    ...
    global_store = _make_auth_store(pool={
        "openai-codex": [
            {"id": "disabled", "label": "global-disabled", "access_token": "old"},
            {"id": "global-ok", "label": "global-ok", "access_token": "good"},
        ],
    })
    global_store["disabled_credential_ids"] = {"openai-codex": ["disabled"]}
    _write(profile_env["global"] / "auth.json", global_store)
    _write(profile_env["profile"] / "auth.json", _make_auth_store(pool={
        "openai-codex": [
            {"id": "disabled", "label": "profile-stale", "access_token": "stale"},
            {"id": "profile-ok", "label": "profile-ok", "access_token": "good"},
        ],
    }))

    assert {row["id"] for row in read_credential_pool("openai-codex")} == {
        "profile-ok", "global-ok",
    }

    # Simulate a long-lived Gateway persisting the stale in-memory row again.
    write_credential_pool("openai-codex", [
        {"id": "disabled", "label": "stale-writer", "access_token": "stale"},
        {"id": "profile-ok", "label": "profile-ok", "access_token": "good"},
    ])
    persisted = json.loads((profile_env["profile"] / "auth.json").read_text())
    assert [row["id"] for row in persisted["credential_pool"]["openai-codex"]] == ["profile-ok"]
